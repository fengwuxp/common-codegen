...
import {API_ADDRESS} from '@/env/EnvVariableConfiguration';
...
class BrowserFeignConfigurationAdapter implements FeignConfigurationAdapter {
  ...
  public registryClientHttpRequestInterceptors = (
    interceptorRegistry: ClientHttpInterceptorRegistry,
  ) => {
    interceptorRegistry.addInterceptor(
      new NetworkClientHttpRequestInterceptor(
        new BrowserNetworkStatusListener(),
        new SimpleNetworkStatusListener(),
      ),
    );
    interceptorRegistry.addInterceptor(
      new RoutingClientHttpRequestInterceptor(API_ADDRESS),
    );
    interceptorRegistry
      .addInterceptor(
        new AuthenticationClientHttpRequestInterceptor(
          new AppAuthenticationStrategy(),
        ),
      )
      .excludePathPatterns('/login', '/api/mock/**');
  };

  public registryFeignClientExecutorInterceptors = (
    interceptorRegistry: FeignClientInterceptorRegistry,
  ) => {
    interceptorRegistry.addInterceptor(
