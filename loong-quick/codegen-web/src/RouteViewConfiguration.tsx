...
import {
  AntdPageHeaderEnhancer,
  AntdRouteViewOptions,
} from 'fengwuxp-routing-antd';
import { ReactCmdDataProviderEnhancer } from 'fengwuxp-routing-react';
import { PageHeaderWrapper } from '@ant-design/pro-layout';
import {
  ANT_DESIGN_GLOBAL_EVENT_PROP_MAP,
  AntDesignGlobalEventNames,
  AntGlobalStateType,
} from '@/AntGlobalEventNames';
...
// 注册全局事件
ReactCmdDataProviderEnhancer.registerGlobalEventNames<
  AntGlobalStateType,
  AntDesignGlobalEventNames
>(ANT_DESIGN_GLOBAL_EVENT_PROP_MAP);
// 增加 cmd 处理
...
AntdPageHeaderEnhancer.setWrapperRender(
  (ReactComponent: any, options: AntdRouteViewOptions, viewProps: any) => {
    console.log('viewProps', viewProps, options);
    const pageHeader = options.pageHeader || {};
    const { location } = viewProps;
    const { state, query } = location;
    if (
      (state && state[ENABLED_MODAL_NAME]) ||
      (query && Boolean(query[ENABLED_MODAL_NAME]))
    ) {
    ...
    return (
      <>
        <PageHeaderWrapper
          title={pageHeader.title || viewProps.route.name}
...
// 设置无权限时的页面渲染
AntdPageHeaderEnhancer.setRenderNoAuthorityView(
  (ReactComponent: any, options: AntdRouteViewOptions, viewProps: any) => {
