package com.wuxp.codegen.meta.transform.retrofit;
 ...
 */
@Slf4j
public class RetrofitRequestMappingTransformer implements
    ...
    static {
        ...
        METHOD_MAPPING_NAME_MAP.put(RequestMethod.DELETE, "DELETE");
    ...
    public CommonCodeGenAnnotation transform(RequestMappingMetaFactory.RequestMappingMate annotationMate, Method annotationOwner) {
        ...
        if (value != null) {
            ...
            log.warn("类：{} 的方法：{} 合并的 url 为 null，请检查控制的注解", annotationOwner.getDeclaringClass().getName(), annotationOwner.getName());
        }
        RequestMethod requestMethod = annotationMate.getRequestMethod();
        if (annotationMate.annotationType().equals(RequestMapping.class)) {
            // 将RequestMapping 转换为其他明确的 Mapping 类型
